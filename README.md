...
Fixed a bug where daily (and other recurring) schedules with auto-posting enabled skipped days during catch-up. When a transaction already existed for a _later_ day in the range (from an earlier run, a manual entry, or out-of-order sync), every earlier unposted occurrence was treated as already "paid" and silently skipped — producing the gaps the user reported.
...
packages/loot-core/src/shared/schedules.test.ts — added unit tests asserting the query's date range is `[matchStart, next_date]`
...
Biggest lesson: the fix was one line conceptually (`$lte: next_date`), but expressing it correctly took reading the query compiler. A single `{ $gte, $lte }` object silently dropped the second operator because the AQL compiler keeps only the first key — the working form is an array `date: [{ $gte }, { $lte }]`, which the compiler AND-s. Always verify how the framework interprets your input, not just what you wrote.
...
Problem
Daily schedules with "post automatically" enabled post transactions irregularly, leaving gaps rather than one transaction per day. A prior fix (#7299, disabling the 2-day lookback) did not fully resolve it.
...
Verify framework semantics — the AQL compiler dropped a silently-ignored operator that looked correct in JavaScript
An existing helper (`isScheduleOccurrencePosted`) already applied the correct upper bound; the query path just never got it
...
**Review:** Will self-review against the repo's `AGENTS.md` and PR/commit conventions (`[AI]` title prefix, blank PR template, release note added) before opening the PR.

**Evaluate:** Manual test reproducing steps 1–3 above should now post a transaction for every day with no gaps. All 3 new tests should fail without the fix and pass with it; all existing tests should continue to pass.

> **Why this works:** Specific setup path documented, branch in the fork, numbered reproduction steps anyone could follow, root cause identified with file/line references, plan tied to the UMPIRE framework with review preparation.
