...
- **Status:** **Closed — duplicate.** The issue had already been fixed and merged upstream by [#8281](https://github.com/actualbudget/actual/pull/8281) before I opened my PR. (Status label: not _Awaiting review / Iterating / Approved / Merged_ — this PR reached a fifth outcome, **closed by me as a duplicate**, which is documented honestly rather than mapped to a label that does not fit.)

**Submission details:**

| Item          | Detail                                                                                                       |
| ------------- | ------------------------------------------------------------------------------------------------------------ |
| Opened        | 2026-06-25 01:10 UTC — **open, not a draft**                                                                 |
| Base branch   | `master` on the **upstream** repo (`actualbudget/actual`), from `Kaydenletk:fix-issue-8134`                  |
| Template used | The **project's own PR template** — `## Description` / `## Related issue(s)` / `## Testing` / `## Checklist` |
| Close keyword | **`Closes #8134`** in the _Related issue(s)_ section (auto-links and would auto-close the issue on merge)    |
| Commit        | `e773b49f2` — [AI] Fix payee rule count excluding schedule-owned rules                                       |

**PR description structure — "why" before "what":** the description opens with two paragraphs of context a reviewer cannot get from the diff (the user-visible symptom, and what tracing the data flow revealed about _where_ the count is computed), and only then describes the change itself. It also carries the required AI-usage disclosure per the project's [AI usage policy](https://actualbudget.org/docs/contributing/ai-usage-policy).
...
### The Full Open Source Loop

This contribution closed the loop end to end, even though the PR itself did not merge:

- **Feedback received and addressed the same day.** Maintainer **@Juulz** replied within 17 minutes; I verified the claim against `master` (commit `208283a51`) rather than taking it at face value, agreed with it, and closed my own PR with an explanatory comment instead of leaving a duplicate for a maintainer to clean up. Automated review (**CodeRabbit**) separately flagged my test-registration pattern.
- **The feedback changed my later work.** CodeRabbit's note about not following the project's test fixture conventions was carried directly into Contribution 2 — I matched the neighbouring test files' setup from the start (commit `dd664aa1e`), so the same comment never recurred.
- **A second contribution cycle is underway.** [Contribution 2](#contribution-2-fix-daily-auto-post-schedules-skipping-days) — issue [#8276](https://github.com/actualbudget/actual/issues/8276), PR [#8418](https://github.com/actualbudget/actual/pull/8418) — is open against upstream `master`, carries the `ready for review` label, and has all CI checks green.
- **The failure produced a transferable lesson.** The "an open issue is not an unfixed issue" insight above came out of losing this cycle, and the pre-claim checklist it produced is what I ran before starting Contribution 2 — which is why #8276 was a live, reproducible bug.

**Knowing when to stop is part of the loop.** The most useful thing I did on this PR was close it. A duplicate left open costs a maintainer triage time on a project that already had 51 open PRs; withdrawing it quickly, with a clear reason, was worth more to the project than my patch would have been.
